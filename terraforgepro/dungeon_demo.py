...
import numpy as np
from dungeonforgepro import DungeonForgePro
import random
...
GLYPH = {
	DungeonForgePro.FLOOR: ".",
	DungeonForgePro.WALL : "#",
	DungeonForgePro.UP   : "<",
	DungeonForgePro.DOWN : ">"
}

z  = 0
ys, xs = np.where(dungeon[0] == DungeonForgePro.UP)
if len(xs) == 0:
	ys, xs = np.where(dungeon[0] == DungeonForgePro.FLOOR)
...
while True:
	...
	if key == "w":	dy = -1
		...
		if dungeon[z][py, px] == DungeonForgePro.DOWN and z < len(dungeon) - 1:
			z += 1
			continue
	elif key == "<":		# go up
		if dungeon[z][py, px] == DungeonForgePro.UP and z > 0:
			z -= 1
			continue
	else:
	...
	if 0 <= ny < level_h and 0 <= nx < level_w:
		if dungeon[z][ny, nx] != DungeonForgePro.WALL:
			py, px = ny, nx
